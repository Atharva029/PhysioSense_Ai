...
async function loadSummaryData() {
  ...
  document.getElementById("exercise-name").textContent = `Remote Rehab Session: ${formatExerciseName(summaryData.exerciseType)}`
  ...
  // Display posture checks
  const postureChecksEl = document.getElementById("posture-checks")
  if (postureChecksEl) {
    postureChecksEl.textContent = summaryData.postureChecks || 0
  }

  // Display incorrect posture %
  const incorrectPercentEl = document.getElementById("incorrect-percent")
  if (incorrectPercentEl) {
    incorrectPercentEl.textContent = `${summaryData.incorrectPercentage || 0}%`
  }
  ...
  // Display risk badge + explanation
  const riskBadge = document.getElementById("risk-summary-badge")
  const riskExplanation = document.getElementById("risk-explanation")
  if (riskBadge) {
    const level = summaryData.riskLevel || "normal"
    riskBadge.className = `risk-badge ${level}`
    riskBadge.textContent = summaryData.riskLabel || "Normal"
  }
  if (riskExplanation) {
    riskExplanation.textContent = summaryData.riskExplanation || "No risk flags detected."
  }

  // Display common mistakes
  const mistakesContainer = document.getElementById("mistakes-content")
  if (mistakesContainer) {
    const mistakes = Array.isArray(summaryData.commonMistakes) ? summaryData.commonMistakes : []
  ...
  if (recEl) {
    recEl.textContent = summaryData.recoveryRecommendation || "-"
  }

  setupSendForReview(summaryData)
...
function setupSendForReview(summaryData) {
  const btn = document.getElementById("send-review-btn")
  const notice = document.getElementById("send-review-notice")
  if (!btn) return
  ...
  btn.addEventListener("click", () => {
    try {
      ...
      const key = "lastSessionForTherapistReview"
      localStorage.setItem(key, JSON.stringify(payload))
      console.log("Session sent for therapist review:", payload)

      if (notice) {
        notice.innerHTML = `<div class="notice success">Session sent for therapist review.</div>`
      }
    } catch (e) {
      console.error("Failed to send session for review:", e)
    }
  })
}
